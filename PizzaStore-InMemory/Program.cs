...
app.MapGet("/pizza/{id}", async (PizzaDb db, int id) =>
    await db.Pizzas.FindAsync(id) is Pizza p ? Results.Ok(p) : Results.NotFound()
);

app.MapPut("/pizza/{id}", async (PizzaDb db, Pizza updated, int id) =>
{
